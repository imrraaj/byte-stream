...
int main(int argc, char **argv) {
    ...
    Texture2D logo_texture =
        bundle_load_texture("./assets/logos/bytestream-bg.png");
    ...
    if (argc > 1) {
        if (player_init(argv[1], &ui_state, &ui_textures) < 0) {
            snprintf(errorMsg, sizeof(errorMsg),
                     "Failed to initialize player with file: %s", argv[1]);
        } else {
            isFileSelected = true;
        }
    }
    ...
    while (!WindowShouldClose() && !should_exit) {
        if (isFileSelected) {
            player_update(&ui_state, &ui_textures);
        } else {
            int screenWidth = GetScreenWidth();
            int screenHeight = GetScreenHeight();
            int padding = 30;
            int logoX = (screenWidth - 256) / 2;
            int logoY = (screenHeight - 256) / 2 - 80;
            ...
            Vector2 textSize1 =
                MeasureTextEx(title_font, select_text, title_size, 1.0f);
            Vector2 text1 = {(screenWidth - textSize1.x) / 2, logoY + 256};
            ...
            if (IsFileDropped()) {
                ...
                for (size_t i = 0; i < droppedFiles.count; i++) {
                    if (GetFileExtension(droppedFiles.paths[i])) {
                        if (player_init(droppedFiles.paths[i], &ui_state, &ui_textures) <
                            0) {
                            snprintf(errorMsg, sizeof(errorMsg),
                                     "Failed to initialize player with file: %s",
            ...
            if (CheckCollisionPointRec(GetMousePosition(), buttonRect)) {
                ...
                if (IsMouseButtonPressed(MOUSE_LEFT_BUTTON)) {
                    char *file_name = tinyfd_openFileDialog(
                        "Please select a video file to play", ".", 1, NULL, NULL, 1);
                    if (file_name) {
                        if (player_init(file_name, &ui_state, &ui_textures) < 0) {
                            snprintf(errorMsg, sizeof(errorMsg),
                                     "Failed to initialize player with file: %s", file_name);
                        } else {
                            isFileSelected = true;
                        }
                    }
            ...
            // resize the texture to fit within 256x256
            DrawTexturePro(logo_texture,
                           (Rectangle){0, 0, logo_texture.width, logo_texture.height},
                           (Rectangle){logoX, logoY, 256, 256}, (Vector2){0, 0}, 0.0f,
                           WHITE);
            ...
            Vector2 buttonTextSize =
                MeasureTextEx(button_font, buttonText, button_size, 0.0f);

            DrawRectangleRec(buttonRect, buttonColor);
            DrawTextEx(
                button_font, buttonText,
                (Vector2){buttonRect.x + (buttonRect.width - buttonTextSize.x) / 2,
            ...
            if (*errorMsg) {
                ...
                Vector2 errorSize =
                    MeasureTextEx(error_font, errorMsg, error_size, 1.0f);
                DrawTextEx(error_font, errorMsg,
                           (Vector2){(screenWidth - errorSize.x) / 2,
