...
Font get_best_font(Custom_Fonts fonts, int target, const char *name) {
    for (size_t i = 0; i < fonts.count; i++) {
        if (fonts.items[i].size == target &&
            strcmp(fonts.items[i].name, name) == 0) {
...
Custom_Fonts init_fonts(void) {
    ...
    int font_sizes[] = {24, 28, 30, 32, 36, 38, 40, 48, 64, 72};
    for (size_t i = 0; i < ARRAY_LEN(font_sizes); i++) {
        Custom_Font sub = {.font = bundle_load_font(SUBTITLE_FONT, font_sizes[i]),
                           .size = font_sizes[i],
                           .name = SUBTITLE_FONT};
        Custom_Font ui = {.font = bundle_load_font(UI_FONT, font_sizes[i]),
                          .size = font_sizes[i],
                          .name = UI_FONT};
        da_append(fonts, sub);
        da_append(fonts, ui);
