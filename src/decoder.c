...
int decoder_decode_frame(void) {
    ...
    if (ds.packet->stream_index == ds.audio_stream_idx) {
        ...
        if (avcodec_send_packet(ds.audio_codec_ctx, ds.packet) == 0) {
            ...
            while (avcodec_receive_frame(ds.audio_codec_ctx,
                                         original_audio_frame) == 0) {
                swr_convert_frame(ds.swr_ctx, resampled_frame,
                                  original_audio_frame);

                pthread_mutex_lock(&ds.queue_mutex);
                double pts_seconds = original_audio_frame->pts *
                                     av_q2d(ds.audio_stream->time_base);
                audio_clock =
                    pts_seconds + (double)resampled_frame->nb_samples /
                                      ds.audio_codec_ctx->sample_rate;
                pthread_mutex_unlock(&ds.queue_mutex);
            }
            av_audio_fifo_write(ds.fifo, (void **)resampled_frame->data,
        ...
        pthread_mutex_unlock(&ds.queue_mutex);
               ...
               ds.subtitle_codec_ctx) {
        AVSubtitle sub;
        int got_frame = 0;
        if (avcodec_decode_subtitle2(ds.subtitle_codec_ctx, &sub, &got_frame,
                                     ds.packet) >= 0 &&
            got_frame) {
            int64_t subtitle_pts_video_timebase =
                av_rescale_q(ds.packet->pts, ds.subtitle_stream->time_base,
                             ds.video_stream->time_base);
            double start_time = subtitle_pts_video_timebase *
                                av_q2d(ds.video_stream->time_base);
            ...
            for (size_t i = 0; i < sub.num_rects; i++) {
                if (sub.rects[i]->type == SUBTITLE_TEXT && sub.rects[i]->text) {
                    add_subtitle(sub.rects[i]->text, start_time, end_time);
                           ...
                           sub.rects[i]->ass) {
                    char *cleaned =
                        clean_subtitle(sub.rects[i]->ass, SUBTITLE_ASS);
                    if (cleaned) {
...
void *video_thread_func(void *arg) {
    ...
    while (ds.decoding) {
        ...
        if (avcodec_send_packet(ds.video_codec_ctx, pkt) == 0) {
            ...
            while (avcodec_receive_frame(ds.video_codec_ctx, video_frame) ==
                   0) {
                double pts_seconds =
                    video_frame->pts * av_q2d(ds.video_stream->time_base);
                ...
                frame_time = (int64_t)(video_frame->pts *
                                       av_q2d(ds.video_stream->time_base));
                int rgba_linesizes[] = {ds.video_codec_ctx->width * 4};
                sws_scale(ds.sws_ctx, (const uint8_t *const *)video_frame->data,
                          video_frame->linesize, 0, ds.video_codec_ctx->height,
                          rgba_planes, rgba_linesizes);
...
int decoder_init(char *filename) {
    ...
    ds.video_stream_idx =
        av_find_best_stream(ds.format_ctx, AVMEDIA_TYPE_VIDEO, -1, -1, NULL, 0);
    ds.audio_stream_idx =
        av_find_best_stream(ds.format_ctx, AVMEDIA_TYPE_AUDIO, -1, -1, NULL, 0);
    ds.subtitle_stream_idx = av_find_best_stream(
        ds.format_ctx, AVMEDIA_TYPE_SUBTITLE, -1, -1, NULL, 0);
    ...
    ds.subtitle_stream = (ds.subtitle_stream_idx >= 0)
                             ? ds.format_ctx->streams[ds.subtitle_stream_idx]
                             : NULL;

    const AVCodec *video_codec =
        avcodec_find_decoder(ds.video_stream->codecpar->codec_id);
    const AVCodec *audio_codec =
        avcodec_find_decoder(ds.audio_stream->codecpar->codec_id);
    const AVCodec *subtitle_codec =
        ds.subtitle_stream
            ? avcodec_find_decoder(ds.subtitle_stream->codecpar->codec_id)
            : NULL;
    ...
    ds.subtitle_codec_ctx =
        subtitle_codec ? avcodec_alloc_context3(subtitle_codec) : NULL;
    ...
    if (avcodec_parameters_to_context(ds.video_codec_ctx,
                                      ds.video_stream->codecpar) < 0 ||
        avcodec_parameters_to_context(ds.audio_codec_ctx,
                                      ds.audio_stream->codecpar) < 0 ||
        (ds.subtitle_codec_ctx &&
         avcodec_parameters_to_context(ds.subtitle_codec_ctx,
                                       ds.subtitle_stream->codecpar) < 0)) {
    ...
    if (avcodec_open2(ds.video_codec_ctx, video_codec, NULL) < 0 ||
        ...
        (ds.subtitle_codec_ctx &&
         avcodec_open2(ds.subtitle_codec_ctx, subtitle_codec, NULL) < 0)) {
    ...
    ds.sws_ctx =
        sws_getContext(ds.video_codec_ctx->width, ds.video_codec_ctx->height,
                       ds.video_codec_ctx->pix_fmt, ds.video_codec_ctx->width,
                       ds.video_codec_ctx->height, AV_PIX_FMT_RGBA,
    ...
    int ret = swr_alloc_set_opts2(
        &ds.swr_ctx, &out_ch_layout, AV_SAMPLE_FMT_S16,
        ds.audio_codec_ctx->sample_rate, &ds.audio_codec_ctx->ch_layout,
        ds.audio_codec_ctx->sample_fmt, ds.audio_codec_ctx->sample_rate, 0,
        NULL);
    ...
    ds.rgba_frame_buffer =
        malloc(ds.video_codec_ctx->width * ds.video_codec_ctx->height * 4);
    memset(ds.rgba_frame_buffer, 0,
           ds.video_codec_ctx->width * ds.video_codec_ctx->height * 4);
...
int decoder_change_audio(char *language) {
    ...
    AVChannelLayout out_ch_layout;
    av_channel_layout_default(&out_ch_layout, 2);
    swr_alloc_set_opts2(&ds.swr_ctx, &out_ch_layout, AV_SAMPLE_FMT_S16,
                        ds.audio_codec_ctx->sample_rate,
                        &ds.audio_codec_ctx->ch_layout,
                        ds.audio_codec_ctx->sample_fmt,
                        ds.audio_codec_ctx->sample_rate, 0, NULL);
    if (swr_init(ds.swr_ctx) < 0) {
    ...
    int64_t seek_target =
        (int64_t)(frame_time / av_q2d(ds.video_stream->time_base));
    int video_seek_result =
        avformat_seek_file(ds.format_ctx, ds.video_stream_idx, INT64_MIN,
                           seek_target, INT64_MAX, AVSEEK_FLAG_BACKWARD);
    int audio_seek_target_ts = av_rescale_q(
        seek_target, ds.video_stream->time_base, ds.audio_stream->time_base);
    int audio_seek_result = avformat_seek_file(
        ds.format_ctx, ds.audio_stream_idx, INT64_MIN, audio_seek_target_ts,
        INT64_MAX, AVSEEK_FLAG_BACKWARD);

    if (video_seek_result < 0 || audio_seek_result < 0) {
        fprintf(
            stderr,
            "ERROR: Seek failed during audio change! Video: %d, Audio: %d\n",
...
int decoder_change_subtitle(char *language) {
    ...
    if (subtitle_streams.count > 0) {
        subtitle_streams.index =
            (subtitle_streams.index + 1) % subtitle_streams.count;
        ds.subtitle_stream_idx =
            subtitle_streams.indices[subtitle_streams.index];
    } else {
...
char *decoder_get_metadata(DecoderState *ds, char *key) {
    while ((ds->tag = av_dict_get(ds->format_ctx->metadata, "", ds->tag,
...
const char *get_current_subtitle(double current_time) {
    ...
    for (SubtitleItem *current = ds.subtitle_queue; current;
         current = current->next) {
        if (current_time >= current->start_time &&
